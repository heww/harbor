...
import (
	...
	"github.com/goharbor/harbor/src/pkg/project/mocks"
...
func (suite *ControllerTestSuite) TestGetByName() {
	...
	mgr.On("Get", mock.AnythingOfType("string")).Return(func(name string) (*models.Project, error) {
		return &models.Project{ProjectID: 1, Name: "library"}, nil
	})
	c := controller{projectMgr: mgr}

	for _, name := range []string{"library", "library/photon"} {
		p, err := c.GetByName(context.TODO(), name)
		suite.Nil(err)
