...
func PostInitiateBlobUploadMiddleware() func(http.Handler) http.Handler {
	...
	return middleware.AfterResponse(func(w http.ResponseWriter, r *http.Request, statusCode int) error {
		...
		ctx := r.Context()
		project, err := projectController.GetByName(ctx, distribution.ParseName(r.URL.Path))
		if err != nil {
			log.Errorf("%s: get project failed, error: %v", logPrefix, err)
			return err
		}

		params := blob.AttachToProjectParams{Digest: mount, ProjectID: project.ProjectID}
		if err := blobController.AttachToProject(ctx, params); err != nil {
