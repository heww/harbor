...
func (suite *PostInitiateBlobUploadMiddlewareTestSuite) TestMountBlob() {
	suite.WithProject(func(projectID int64, projectName string) {
		...
		_, err := blob.Ctl.Ensure(ctx, blob.EnsureParams{Digest: digest, Size: 512})
		suite.Nil(err)
		...
		suite.WithProject(func(id int64, name string) {
			...
			exist, err := blob.Ctl.Exist(ctx, blob.ExistParams{Digest: digest, ProjectID: id})
			suite.Nil(err)
			suite.True(exist)

			blob, err := blob.Ctl.Get(ctx, blob.GetParams{Digest: digest})
			if suite.Nil(err) {
