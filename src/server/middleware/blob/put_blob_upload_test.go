...
func (suite *PutBlobUploadMiddlewareTestSuite) TestDataInBody() {
	suite.WithProject(func(projectID int64, projectName string) {
		...
		exist, err := blob.Ctl.Exist(suite.Context(), blob.ExistParams{Digest: digest, ProjectID: projectID})
		suite.Nil(err)
		suite.True(exist)

		blob, err := blob.Ctl.Get(suite.Context(), blob.GetParams{Digest: digest})
		suite.Nil(err)
